...
# near_ten(19) -> True
def near_ten(n):
  return(n+2)%10==0 or (n+1)%10==0 or (n-2)%10==0 or (n-1)%10==0 or n%10==0
...
def count_code(s):
  ...
  for n in range(3, len(s)):
    if s[n-3] + s[n-2]=='co' and s[n-1] in alf and s[n]=='e':
      c+=1
  return c
...
def fila_tijolos(n_peq, n_gra, meta):
  return meta%5 >= 0 and meta%5 - n_peq <= 0 and n_peq + 5*n_gra >= meta
